...
import { ActivityIndicator, Alert, FlatList, Modal, StyleSheet, Text, TextInput, TouchableOpacity, View, Platform, KeyboardAvoidingView, Image } from 'react-native';
...
export default function ProductsScreen() {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            console.error(e);
            Alert.alert(language === 'Tamil' ? 'பிழை' : 'Error', language === 'Tamil' ? 'பொருட்களை ஏற்றுவதில் தோல்வி' : 'Failed to load products');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        if (!name.trim()) {
            Alert.alert(language === 'Tamil' ? 'சரிபார்ப்பு பிழை' : 'Validation Error', language === 'Tamil' ? 'பொருள் பெயர் அவசியம்' : 'Product Name is required');
            return;
        }
        ...
        try {
            ...
            console.error(e);
            Alert.alert(language === 'Tamil' ? 'பிழை' : 'Error', language === 'Tamil' ? 'பொருளைச் சேமிப்பதில் தோல்வி' : 'Failed to save product');
        }
    };

    const handleDelete = (id: string) => {
        Alert.alert(language === 'Tamil' ? 'நீக்க உறுதிப்படுத்தவும்' : 'Confirm Delete', language === 'Tamil' ? 'இந்த பொருளை நீக்க விரும்புகிறீர்களா?' : 'Are you sure you want to delete this product?', [
            { text: language === 'Tamil' ? 'ரத்து' : 'Cancel', style: 'cancel' },
            {
                text: language === 'Tamil' ? 'நீக்கு' : 'Delete', style: 'destructive', onPress: async () => {
                    try {
                        ...
                        Alert.alert(language === 'Tamil' ? 'பிழை' : 'Error', language === 'Tamil' ? 'பொருளை நீக்குவதில் தோல்வி' : 'Failed to delete product');
                    }
                }
            }
        ]);
        ...
        >
            <View style={styles.cardContent}>
                ...
                <View style={styles.productInfo}>
                    <Text style={[styles.tamilTitle, { color: textColor }]}>{item.tamil_name || item.name}</Text>
                    <Text style={[styles.engTitle, { color: subTextColor }]}>{item.name}</Text>
                    {item.category ? (
                        <View style={[styles.catBadge, { backgroundColor: primaryColor + '10' }]}>
                ...
                <View style={styles.actionGroup}>
                    ...
                    <TouchableOpacity onPress={() => item.id && handleDelete(item.id)} style={styles.iconBtn}>
        ...
        <View style={[styles.container, { backgroundColor: background }]}>
            ...
            >
                ...
                <View style={styles.headerTop}>
                    <View>
                        <Text style={styles.headerTitle}>{language === 'Tamil' ? 'பொருட்கள்' : 'Products'}</Text>
                        <Text style={styles.headerSubtitle}>{products.length} {language === 'Tamil' ? 'பொருட்கள் பட்டியலில் உள்ளன' : 'items in inventory'}</Text>
                    </View>
            ...
            {isLoading ? (
                ...
                <View style={styles.emptyBox}>
                    ...
                    <Text style={[styles.emptyTitle, { color: textColor }]}>{language === 'Tamil' ? 'பொருட்கள் எதுவும் இல்லை' : 'No products yet'}</Text>
                    <Text style={[styles.emptyDesc, { color: subTextColor }]}>{language === 'Tamil' ? 'விற்பனையைத் தொடங்க உங்கள் சரக்கு பொருட்களை இங்கே சேர்க்கவும்.' : 'Add your inventory items to start billing them on the PoS screen.'}</Text>
                    <TouchableOpacity style={[styles.emptyAddBtn, { backgroundColor: primaryColor }]} onPress={openAddModal}>
                        <Text style={styles.emptyAddBtnText}>{language === 'Tamil' ? 'முதல் பொருளைச் சேர்க்கவும்' : 'Add First Product'}</Text>
                    </TouchableOpacity>
                </View>
            ) : (
                <FlatList
                ...
                >
                    ...
                    >
                        ...
                        <Text style={[styles.sheetTitle, { color: textColor }]}>
                            {editingProduct ? (language === 'Tamil' ? 'பொருளைப் புதுப்பி' : 'Update Product') : (language === 'Tamil' ? 'புதிய பொருள்' : 'New Product')}
                        </Text>

                        <View style={styles.inputWrapper}>
                            <Text style={[styles.inputLabel, { color: subTextColor }]}>{language === 'Tamil' ? 'பொருள் பெயர் (ஆங்கிலம்)' : 'Product Name (English)'}</Text>
                            <TextInput 
                                style={[styles.input, { color: textColor, borderColor: isDark ? '#333' : '#E2E8F0', backgroundColor: isDark ? '#2C2C2C' : '#F8FAFC' }]} 
                                value={name} 
                                onChangeText={setName} 
                                placeholder={language === 'Tamil' ? 'எ.கா. தக்காளி' : "e.g. Tomato"} 
                                placeholderTextColor="#94A3B8"
                            />
                        </View>

                        <View style={styles.inputWrapper}>
                            <Text style={[styles.inputLabel, { color: subTextColor }]}>{language === 'Tamil' ? 'பொருள் பெயர் (தமிழ்)' : 'Regional Name (Tamil)'}</Text>
                            <TextInput 
                                style={[styles.input, { color: textColor, borderColor: isDark ? '#333' : '#E2E8F0', backgroundColor: isDark ? '#2C2C2C' : '#F8FAFC' }]} 
                                value={tamilName} 
                                onChangeText={setTamilName} 
                                placeholder={language === 'Tamil' ? 'எ.கா. தக்காளி' : "எ.கா. தக்காளி"}
                                placeholderTextColor="#94A3B8"
                            />
                        </View>

                        <View style={styles.inputWrapper}>
                            <Text style={[styles.inputLabel, { color: subTextColor }]}>{language === 'Tamil' ? 'வகை' : 'Category'}</Text>
                            <TextInput 
                                style={[styles.input, { color: textColor, borderColor: isDark ? '#333' : '#E2E8F0', backgroundColor: isDark ? '#2C2C2C' : '#F8FAFC' }]} 
                                value={category} 
                                onChangeText={setCategory} 
                                placeholder={language === 'Tamil' ? 'காய்கறிகள், கீரைகள் போன்றவை...' : "Essentials, Roots, etc."}
                                placeholderTextColor="#94A3B8"
                            />
                        </View>
                        ...
                        <View style={styles.sheetActions}>
                            ...
                            <TouchableOpacity style={[styles.sheetBtn, styles.saveSheetBtn, { backgroundColor: primaryColor }]} onPress={handleSave}>
                                <Text style={styles.saveSheetText}>{editingProduct ? (language === 'Tamil' ? 'புதுப்பிக்கவும்' : 'Update') : (language === 'Tamil' ? 'சேர்க்கவும்' : 'Add Item')}</Text>
                            </TouchableOpacity>
                        </View>
                    </Animated.View>
                </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
    ...
    productInfo: { flex: 1 },
    tamilTitle: { fontSize: moderateScale(17), fontWeight: '800', marginBottom: 1 },
    engTitle: { fontSize: moderateScale(12), fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: verticalScale(6) },
    catBadge: {
    ...
    catBadgeText: { fontSize: moderateScale(10), fontWeight: '800', textTransform: 'uppercase' },
