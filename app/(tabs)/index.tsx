...
// Mock function to simulate "Top Selling" - in real app this comes from backend/signup
const TOP_SELLING_IDS = ['1', '12', '13', '15', '29', '32', '39', '40'];
...
export default function BillingScreen() {
  ...
  // Merge logic: Put Top Selling first, then the rest
  const sortedVegetables = useMemo(() => {
    const top = SOUTHERN_VEGETABLES.filter(v => TOP_SELLING_IDS.includes(v.id));
    const others = SOUTHERN_VEGETABLES.filter(v => !TOP_SELLING_IDS.includes(v.id));
  ...
  const renderVegetableItem = ({ item, index }: { item: Vegetable, index: number }) => {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#121212' : '#F5FDF7' }]}>
      ...
      <View style={styles.searchContainer}>
        <View style={[styles.searchBar, { backgroundColor: isDark ? '#2C2C2C' : '#FFF' }]}>
      ...
      {cartCount > 0 && (
        ...
        >
          <View style={[styles.bottomBar, { backgroundColor: isDark ? '#1E1E1E' : '#FFF' }]}>
            <View style={styles.totalInfo}>
              ...
              <View>
                ...
                <Text style={[styles.totalValue, { color: isDark ? '#FFF' : '#333' }]}>₹{cartTotal}</Text>
              </View>
            </View>
